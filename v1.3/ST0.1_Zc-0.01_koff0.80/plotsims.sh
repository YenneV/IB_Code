...
vscale=1.96078
output1="Plot"$number".eps" 

#calls gnuplot, but leaves the input file unfinished 
gnuplot -persist <<EOF  
...
set cblabel "Concentration/Steady State Concentration" 
set xrange [-60:60] 
set yrange [-60:60] 
set xlabel "x (R_0)" 
set xlabel "y (R_0)" 
plot "$input1" u 1:2:(\$5*\$9/2.22886) palette pt 5 ps 2 t '', "$input1" u 1:2:(\$3*$vscale):(\$4*$vscale) w vectors lc 3 t '', "$input2" u 1:2:(\$5/1) palette pt 5 ps 1 t ''
EOF